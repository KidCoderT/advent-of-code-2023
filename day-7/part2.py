...
for deal in input:
    ...
    if "J" in cards:
        possible_combos = [cards] + [cards.replace("J", letter) for letter in cards.replace("J", "")]
        cards = list(sorted(possible_combos, key=lambda x: sum([x.count(l) for l in x])))[-1]
...
def compare(deck1, deck2):
    ...
    mapping = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
